/* ============================================================
   ...
   homeUrl, detailsUrl, customizeUrl, customizeIndexUrl, billingUrl)
============================================================ */
(function () {
    "use strict";
    ...
    function isSelected(lineId) {
        return CF.getSelectedIds(CF.getCart()).has(lineId);
    }
    ...
    function renderCart() {
        ...
        const selectedIds = CF.getSelectedIds(cart);

        listEl.innerHTML = cart.map(c => {
            const productOnlyPrice = Number(c.price);
            ...
            <div class="cf-cart-item">
                ...
                <div class="cf-cart-clickable" onclick="cfViewCartItem(event, '${c.lineId}')" title="View item details">
                    ...
                    <div class="cf-cart-info">
                        ...
                        <div class="cf-cart-price">${CF.peso(productOnlyPrice)}</div>
                    </div>
                </div>
    ...
    function updateSummary(cart, selectedIds) {
        ...
        const productTotal = selectedLines.reduce((s, c) => s + Number(c.price) * (c.qty || 1), 0);
        const serviceTotal = selectedLines.reduce((s, c) => s + Number(c.servicePrice || 0) * (c.qty || 1), 0);
        const subtotal = productTotal + serviceTotal;

        //document.getElementById("cartProductPrice").textContent = CF.peso(productTotal);

        //const serviceRow = document.getElementById("cartServiceFeeRow");
        //if (serviceTotal > 0) {
        //    serviceRow.style.display = "";
        //    document.getElementById("cartServiceFee").textContent = CF.peso(serviceTotal);
        //} else {
    ...
    function cfToggleLine(lineId, checked) {
        CF.setSelectedId(lineId, checked);
        renderCart();
    }

    function cfToggleSelectAll(checked) {
        CF.selectAllIds(checked);
        renderCart();
    }
    ...
    function cfCheckout() {
        ...
        // Clear any leftover "back to item details" override from a
        // previous Buy Now flow — checking out from the Cart page itself
        // means "Back" on Billing should go back to the Cart, not to
        // whatever single item page was last visited.
        try { sessionStorage.removeItem("cf_checkout_back_url"); } catch (e) { /* ignore */ }

        const pageData = window.CF_CART_PAGE || {};
        window.location.href = pageData.billingUrl || "/Home/hBilling";
    }
