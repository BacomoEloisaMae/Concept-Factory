...
(function () {
    ...
    // Checked via the .js-hidden class (not wrapper.style.display) — the
    // view now toggles this with a conditional CSS class instead of an
    // inline style, since whether there's an existing image is decided
    // server-side at render time.
    if (wrapper && !wrapper.classList.contains('js-hidden')) {
...
(function () {
    ...
    function render() {
        ...
        variants.forEach((v, i) => {
            ...
            cam.className = 'color-picker-swatch-cam';
