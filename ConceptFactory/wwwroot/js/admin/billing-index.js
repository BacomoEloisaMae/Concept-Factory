...
// "Reject Payment" confirmation modal — same idea as Mark Fully Paid above,
// but requires a reason (typed into the modal, not the optional inline
// Notes field, which is Approve-only) before the form is actually allowed
// to submit.
const rejectConfirmBackdrop = document.getElementById('rejectConfirmBackdrop');
let rejectPendingForm = null;

function closeRejectConfirm() {
    rejectConfirmBackdrop.classList.remove('open');
    rejectPendingForm = null;
}

function confirmReject() {
    if (!rejectPendingForm) return;
...
document.addEventListener('submit', function (e) {
    ...
    if (form.matches('[data-confirm-reject]')) {
        e.preventDefault();
        rejectPendingForm = form;
        document.getElementById('rejectReasonInput').value = '';
        document.getElementById('rejectReasonError').classList.remove('show');
        rejectConfirmBackdrop.classList.add('open');
        return;
    }
...
document.addEventListener('keydown', function (e) {
    ...
    if (e.key === 'Escape' && rejectConfirmBackdrop.classList.contains('open')) { closeRejectConfirm(); return; }
    if (e.key === 'Escape' && paymentModalBackdrop.classList.contains('open')) closePaymentModal();
});

// Copies the (optional) note textarea into the hidden "note" input for the
// Approve form — lives here (not in the AJAX-loaded partial) because
// <script> tags injected via innerHTML don't execute. Also copies the Cash
// "Amount Received" input into its form's hidden field and blocks
// submission if it's short of the required down payment.
