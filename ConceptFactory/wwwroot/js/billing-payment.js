...
(function () {
    ...
    function bpSubmitOrder() {
        ...
        // Read Billing Information straight from the input fields — in
        // Sales mode (walk-in checkout) these are editable and typed in
        // live; for a logged-in customer they're readonly but still
        // regular inputs, so this works the same either way.
        const billingName = (document.getElementById("bpBillingName") || {}).value || "";
        const billingEmail = (document.getElementById("bpBillingEmail") || {}).value || "";
        const billingPhone = (document.getElementById("bpBillingPhone") || {}).value || "";
        const billingAddress = (document.getElementById("bpBillingAddress") || {}).value || "";
        const isSalesMode = !!(window.CF_BILLING_PAGE || {}).isSalesMode;

        if (isSalesMode && (!billingName.trim() || !billingPhone.trim())) {
            CF.toast("Enter the customer's name and phone number", { icon: "fa-triangle-exclamation", type: "info" });
            return;
        }
        ...
        formData.append("fullName", billingName);
        formData.append("email", billingEmail);
        formData.append("phone", billingPhone);
        ...
        fetch((window.CF_BILLING_PAGE || {}).submitUrl || "/Home/SubmitOrder", { method: "POST", body: formData })
            ...
            .then(({ status, ok, text }) => {
                ...
                if (!ok || !data.success) {
                    if (data.requireLogin && data.redirectUrl) {
                        CF.toast(data.message || "Please log in to check out.", { icon: "fa-user-lock", type: "info" });
