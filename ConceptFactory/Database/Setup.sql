...
BEGIN
    ...
    -- NOTE: no seed row here on purpose. The real admin login lives only
    -- in [AdminUsers] below (admin@conceptfactory.com / admin123) — this
    -- table is for Staff (RoleID = Staff) accounts created from the admin
    -- panel's User Management screen. An earlier version of this seed
    -- inserted a second "admin@conceptfactory.com" row here with RoleID=1
    -- (Admin) and a different password ("Admin@123"); AuthController's
    -- staff lookup only ever matches RoleName = 'Staff', so that row was
    -- never actually reachable for login — it just caused confusion about
    -- which password was the real one. Removed.
    PRINT 'Users table created.';
END
GO

-- Existing installs created before Department/IsDeleted/DeletedAt existed
-- (added when User Management → Staff CRUD was built; Department is only
-- meaningful for RoleID = Staff — 'Sales' or 'Production').
...
-- Login security (CIA — account lockout, see Utils/AccountSecurity.cs):
-- 3 wrong passwords in a row locks a staff account for 10 seconds.
...
-- this staff member is assigned to — only meaningful for Department =
-- 'Production'. Drives the "My Station" sidebar restriction and
-- per-station notification scoping.
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[dbo].[Users]') AND name = 'StationIndex')
    ALTER TABLE [dbo].[Users] ADD StationIndex INT NULL;
GO
...
-- Login security (CIA — account lockout, see Utils/AccountSecurity.cs):
-- 3 wrong passwords in a row locks the admin account for 10 seconds. The
-- Password column itself is hashed in place at app startup (see Utils/
-- PasswordMigration.cs) — the seeded 'admin123' below stays the login
-- password from the admin's point of view either way.
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[dbo].[AdminUsers]') AND name = 'FailedLoginAttempts')
    ALTER TABLE [dbo].[AdminUsers] ADD FailedLoginAttempts INT NOT NULL DEFAULT 0;
GO
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[dbo].[AdminUsers]') AND name = 'LockoutEnd')
    ALTER TABLE [dbo].[AdminUsers] ADD LockoutEnd DATETIME NULL;
GO

-- Real customer accounts (register/login at /Account) — required to check
-- out (see Filters/CustomerAuthFilter.cs), never required just to browse.
...
-- Existing installs created before these columns existed — without this,
-- EF throws "Invalid column name" for whichever of these the DB predates,
-- since the CREATE TABLE above only runs the very first time the table
-- doesn't exist at all.
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[dbo].[Customers]') AND name = 'Address')
    ALTER TABLE [dbo].[Customers] ADD Address NVARCHAR(300) NULL;
GO
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[dbo].[Customers]') AND name = 'FailedLoginAttempts')
    ALTER TABLE [dbo].[Customers] ADD FailedLoginAttempts INT NOT NULL DEFAULT 0;
GO
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[dbo].[Customers]') AND name = 'LockoutEnd')
    ALTER TABLE [dbo].[Customers] ADD LockoutEnd DATETIME NULL;
GO
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[dbo].[Customers]') AND name = 'ResetCode')
    ALTER TABLE [dbo].[Customers] ADD ResetCode NVARCHAR(10) NULL;
GO
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[dbo].[Customers]') AND name = 'ResetCodeExpiry')
    ALTER TABLE [dbo].[Customers] ADD ResetCodeExpiry DATETIME NULL;
GO
...
GO
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[dbo].[Customers]') AND name = 'CreatedAt')
    ALTER TABLE [dbo].[Customers] ADD CreatedAt DATETIME NOT NULL DEFAULT GETDATE();
GO
...
-- One-time cleanup for databases created before the fix above: an old
-- version of this script also inserted a second, unreachable
-- "admin@conceptfactory.com" row into [Users] with RoleID = 1 (Admin)
-- and password "Admin@123" — different from the real admin login in
-- [AdminUsers] ("admin123"). Staff login only ever matches RoleID =
-- Staff, so that row was dead weight that just caused login confusion.
-- Safe to run repeatedly: does nothing once the row is gone.
DELETE u FROM [dbo].[Users] u
INNER JOIN [dbo].[Roles] r ON r.RoleID = u.RoleID
WHERE u.Email = 'admin@conceptfactory.com' AND r.RoleName = 'Admin';
GO
...
-- Links an order to the Customer account that placed it (see Models/
-- Customer.cs, AccountController). Nullable so orders placed before
-- customer accounts existed keep working — CustomerName/Email/Phone/
-- Address are still what every screen displays either way.
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[dbo].[Orders]') AND name = 'CustomerID')
...
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'FK_Orders_Customers')
    ALTER TABLE [dbo].[Orders] ADD CONSTRAINT FK_Orders_Customers
        FOREIGN KEY (CustomerID) REFERENCES [dbo].[Customers](CustomerID) ON DELETE SET NULL;
GO
...
-- Optional target completion date, set by admin from the Production
-- overview table (ProductionController.pSetTargetDate). No default —
-- stays NULL ("Not set") until someone deliberately picks a date.
IF NOT EXISTS (
    SELECT 1
    FROM sys.columns
    WHERE object_id = OBJECT_ID(N'[dbo].[Orders]')
      AND name = 'ProductionTargetDate'
)
BEGIN
    ALTER TABLE [dbo].[Orders]
    ADD ProductionTargetDate DATETIME NULL;

    PRINT 'ProductionTargetDate added.';
END
GO
...
-- ============================================================
-- In-app notifications — one row per Admin- or Customer-audience event,
-- written by NotificationService (Utils/) from the same points
-- ActivityLogger already logs from: order placed, billing verified/
-- rejected/fully paid, each production stage change, and completed/
-- picked up. Customer rows are matched to a visitor at read time via
-- the linked Order's CustomerEmail/CustomerPhone (see
-- HomeController.hTrackOrder/hNotifications) — same as Track Order,
-- since there are no real customer accounts yet.
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Notifications]') AND type = N'U')
BEGIN
    CREATE TABLE [dbo].[Notifications] (
        NotificationID INT IDENTITY(1,1) PRIMARY KEY,
        OrderID        INT NOT NULL REFERENCES [dbo].[Orders](OrderID) ON DELETE CASCADE,
        Audience       NVARCHAR(20) NOT NULL,   -- 'Admin' | 'Customer'
        Type           NVARCHAR(40) NOT NULL,   -- see Models/NotificationDisplay.cs
        Title          NVARCHAR(150) NOT NULL,
        Message        NVARCHAR(300) NOT NULL,
        CreatedAt      DATETIME NOT NULL DEFAULT GETDATE(),
        IsRead         BIT NOT NULL DEFAULT 0
    );
    CREATE INDEX IX_Notifications_Audience_CreatedAt ON [dbo].[Notifications] (Audience, CreatedAt DESC);
    CREATE INDEX IX_Notifications_OrderID ON [dbo].[Notifications] (OrderID);
    PRINT 'Notifications table created.';
END
GO

-- Which production station (0-5, see Utils/ProductionStations.cs) a
-- production-related notification concerns — null for non-production
-- types (OrderPlaced, PaymentVerified, etc). Lets Production staff see
-- only notifications for their own assigned station.
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID(N'[dbo].[Notifications]') AND name = 'StationIndex')
    ALTER TABLE [dbo].[Notifications] ADD StationIndex INT NULL;
GO
...
-- SECTION 7 — PAYMENTS
-- ============================================================
-- Append-only audit trail of every payment event on an order — the down
-- payment, any rejected/resubmitted attempt, and the later balance
-- settlement each get their own row. Orders keeps its own PaymentStatus/
-- DownPaymentAmount/RemainingBalance/ReferenceNumber/ProofFilePath fields
-- untouched — every existing screen still reads the current state from
-- there. This table only adds the "who verified what, and when" history
-- (written from BillingController.bApprove/bReject/bMarkFullyPaid and
-- HomeController checkout/hReuploadProof).
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Payments]') AND type = N'U')
BEGIN
    CREATE TABLE [dbo].[Payments] (
        PaymentID          INT IDENTITY(1,1) PRIMARY KEY,
        OrderID            INT NOT NULL REFERENCES [dbo].[Orders](OrderID) ON DELETE CASCADE,
        PaymentType        NVARCHAR(30) NOT NULL DEFAULT 'Down Payment', -- 'Down Payment' | 'Balance Settlement'
        Amount             DECIMAL(10,2) NOT NULL,
        PaymentMethod      NVARCHAR(30) NULL,   -- 'Gcash' | 'Cash'
        ReferenceNumber    NVARCHAR(100) NULL,
        ProofFilePath      NVARCHAR(500) NULL,
        CashAmountReceived DECIMAL(10,2) NULL,
        PaymentStatus      NVARCHAR(30) NOT NULL DEFAULT 'Waiting for Verification',
...
-- ============================================================
-- SECTION 8 — PRODUCTION TRACKING
-- ============================================================
-- Append-only history of every production-stage transition an order goes
-- through — one row per stage change. Orders keeps its own
-- ProductionStage/ProductionStageUpdatedAt/ProductionRemarks fields
-- untouched (see ProductionWorkflow.SetStage) as the "current state" every
-- Production screen reads from; this table adds the timeline those
-- single-row columns can't hold (written from ProductionController's
-- pSaveStationProgress/pSetStage/pMarkCompleted).
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ProductionTracking]') AND type = N'U')
BEGIN
    CREATE TABLE [dbo].[ProductionTracking] (
        ProductionTrackingID INT IDENTITY(1,1) PRIMARY KEY,
        OrderID              INT NOT NULL REFERENCES [dbo].[Orders](OrderID) ON DELETE CASCADE,
        ProductionStage      INT NOT NULL,
        StageName            NVARCHAR(50) NULL,
        StageStatus          NVARCHAR(30) NOT NULL DEFAULT 'Reached', -- 'Reached' | 'Completed'
        UpdatedByName        NVARCHAR(200) NULL,
        UpdatedAt            DATETIME NOT NULL DEFAULT GETDATE(),
        Remarks              NVARCHAR(500) NULL
    );
    CREATE INDEX IX_ProductionTracking_OrderID ON [dbo].[ProductionTracking] (OrderID);
    PRINT 'ProductionTracking table created.';
END
GO


-- ============================================================
-- SECTION 9 — REPORT LOGS
-- ============================================================
-- Accountability trail for Reports & Analytics — one row every time
-- someone views/generates a report, recording who, which report type, and
-- what date range. The report numbers themselves stay fully live/computed
-- (see ReportsController.rIndex) — this table only answers "who looked at
-- this report, and when."
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ReportLogs]') AND type = N'U')
BEGIN
    CREATE TABLE [dbo].[ReportLogs] (
        ReportLogID    INT IDENTITY(1,1) PRIMARY KEY,
        ReportType     NVARCHAR(30) NOT NULL,   -- 'Sales' | 'Billing' | 'Orders' | 'Production'
        DateFrom       DATETIME NOT NULL,
        DateTo         DATETIME NOT NULL,
        GeneratedByName NVARCHAR(200) NULL,
        GeneratedAt    DATETIME NOT NULL DEFAULT GETDATE()
    );
    CREATE INDEX IX_ReportLogs_GeneratedAt ON [dbo].[ReportLogs] (GeneratedAt DESC);
    PRINT 'ReportLogs table created.';
END
GO


-- ============================================================
-- SECTION 10 — CART ITEMS / WISHLIST ITEMS
-- ============================================================
-- Account-tied storefront data. Guests (not logged in) still work purely
-- off localStorage in storefront.js, unchanged. Once a Customer is logged
-- in, storefront.js also syncs here (HomeController hGetCart/hSyncCart/
-- hGetWishlist/hSyncWishlist), so cart + wishlist follow the account
-- across logout/login and across devices instead of being stuck in one
-- browser's localStorage.
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[CartItems]') AND type = N'U')
BEGIN
    CREATE TABLE [dbo].[CartItems] (
        CartItemID      INT IDENTITY(1,1) PRIMARY KEY,
        CustomerID      INT NOT NULL REFERENCES [dbo].[Customers](CustomerID) ON DELETE CASCADE,
        LineKey         NVARCHAR(300) NULL,
        ProductID       INT NOT NULL,
        ...
        PrintLocation   NVARCHAR(100) NULL,
        StockQuantity   INT NULL,
        IsCustomOrder   BIT NOT NULL DEFAULT 0,
        DesignImage     NVARCHAR(MAX) NULL,
        LocationNote    NVARCHAR(500) NULL,
        HasFrontDesign  BIT NOT NULL DEFAULT 0,
        HasBackDesign   BIT NOT NULL DEFAULT 0,
        DesignCount     INT NOT NULL DEFAULT 0,
        CreatedAt       DATETIME NOT NULL DEFAULT GETDATE(),
        UpdatedAt       DATETIME NOT NULL DEFAULT GETDATE()
    );
    CREATE INDEX IX_CartItems_CustomerID ON [dbo].[CartItems] (CustomerID);
    PRINT 'CartItems table created.';
END
GO

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[WishlistItems]') AND type = N'U')
BEGIN
    ...
    CREATE INDEX IX_WishlistItems_CustomerID ON [dbo].[WishlistItems] (CustomerID);
    PRINT 'WishlistItems table created.';
END
GO
...
--USE ConceptFactoryDB;
--GO
--SELECT OrderID, Status, ProductionTargetDate
--FROM Orders
--WHERE OrderID = 2045;
...
---- ============================================
---- DELETE TRANSACTION / OPERATIONAL DATA ONLY
---- ============================================
...
---- ============================================
---- RE-ENABLE AND VALIDATE FOREIGN KEYS
---- ============================================
...
--PRINT 'Products, Services, Categories, Roles, AdminUsers, Users and product data were NOT deleted.';
--GO