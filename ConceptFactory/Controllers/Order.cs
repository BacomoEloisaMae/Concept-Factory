...
namespace ConceptFactory.Models
{
    ...
    public class Order
    {
        ...
        // Set for every order placed while logged in as a Customer (which
        // is now required at checkout — see CustomerAuthFilter). Nullable
        // only so orders placed before customer accounts existed keep
        // working; the CustomerName/Email/Phone/Address snapshot fields
        // below are still what every screen displays either way.
        ...
        // Optional target completion date, set by admin from the
        // Production overview table (ProductionController.pSetTargetDate).
        // Null until someone sets it — there's no automatic default,
        // since guessing one and quietly showing it as if it were
        // deliberate would be misleading.
