...
namespace ConceptFactory.Controllers
{
    ...
    public class OrdersController : Controller
    {
        ...
        // status change Sales/Admin can make directly).
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> oUpdateStatus(int id, string status, string? returnUrl)
        {
            var order = await _context.Orders.FirstOrDefaultAsync(o => o.OrderID == id);
            if (order != null)
            {
                ...
                else
                {
                    ...
                    await ActivityLogger.LogAsync(_context, HttpContext, "Orders", $"Marked Order {status}",
                        $"{order.CustomerName} — {previousStatus} → {status}.", order.OrderID);
